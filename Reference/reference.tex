...
\begin{enumerate}
  ...
  \item $\boldsymbol{inst}$: Instantiate the flexible generic values (from step one) to values by \emph{unification}.
\end{enumerate}
...
For variable patterns ($x$) and inaccessible patterns (\underline{$e$}), they are converted to flexible generic values in $\boldsymbol{patternToVal}$. 

$\boldsymbol{inst}$ is the function that instantiates these to values. That is, $inst$ returns a mapping of generic values to values, which is called a \emph{substitution}. 
