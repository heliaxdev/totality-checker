...
\textbf{The first phase} comprises of the following steps:

\begin{enumerate}
  \item Convert patterns to values. Variable patterns and inaccessible patterns are converted to \emph{flexible generic values}.
  \item Instantiate the flexible generic values from step one to values by \emph{unification}.
  \item Check the patterns, in short:
        \begin{itemize}
          \item variable patterns: instantiate the variable to a generic value and add it to the environments.
          \item constructor patterns: instantiate the constructor pattern to values by unification.
          \item inaccessible patterns: instantiate the inaccessible pattern to a generic value and move on to the next generic value.
        \end{itemize}
\end{enumerate}
