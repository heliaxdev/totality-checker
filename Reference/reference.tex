...
the right hand side can be evaluated (by $matchClause$). 

The sequence of clauses is then matched against the evaluated right hand side values (by $matchClauses$). Each clause is tried until one is matched or there are no clauses left. 
...
Pattern matching can fail and no environment is returned in such case. $match$ is used in $matchClause$, which evaluates and returns the type of the right hand side expression of a given clause.
